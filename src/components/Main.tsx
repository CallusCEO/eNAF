...
export default function Main() {
	...
	const searchCompany = async (companyName: string) => {
		try {
			...
			if (!response.ok) {
				...
				const errorMessage =
					responseData?.error || responseData?.message || 'Failed to fetch company data';
	...
	const processAllSecondChance = async (
		...
		for (const mail of list) {
			...
			while (mail.company.length + i !== 0) {
				...
				console.log(
					`i = ${i} and condition = ${mail.company.length + i !== 0} calls = `,
					callsRef.current
				);
	...
	const readFile = async () => {
		...
		try {
			if (checkEmail(email)) {
				...
				const data: DataType[] = [];
				const secondChanceArrGlobal: { name: string; domain: string; company: string }[] =
					[];
	...
	const buttonHandlingUI = () => {
		if (isProcessing) {
			...
			return (
				<button onClick={() => readFile()} className={styles.productButton}>
					Process
		...
		return (
			<button onClick={() => fileInputRef.current?.click()} className={styles.productButton}>
	...
	const textHandlingUI = () => {
		if (isProcessing && !message) {
			return (
				<Text variant='shine' className={styles.productTextBodyProcessing}>
					Your {checkEmail(email) ? 'email' : 'file'} is being processed...
				</Text>
			);
		} else if (message) {
			return (
		...
		<div className={styles.container}>
			...
			<div className={styles.productContainer}>
				<div className={styles.productTitleContainer}>
					<Text variant='generate-effect' className={styles.productTitle}>
						Drop your mails
					</Text>
					<Email
						size={24}
				...
				<div className={styles.productBox}>
					<div className={styles.productHeader}>
						<Text variant='generate-effect' className={styles.productTitleHeader}>
							Choose a method
						</Text>
					...
					<div
						...
						style={
							isDragActive
								? {
										backgroundColor: 'rgba(98, 176, 255, 0.1)',
										borderColor: '#62b0ff',
								  }
								: undefined
						}
					>
									...
									<TableBody>
										{dataMails.map((item: DataType, index) => (
											<TableRow key={index}>
												...
												<TableCell>
													{item.naf === '' ? 'Not found' : item.naf}
												</TableCell>
												<TableCell>
													{item.siret === '' ? 'Not found' : item.siret}
												</TableCell>
												<TableCell>
													{item.siren === '' ? 'Not found' : item.siren}
												</TableCell>
												<TableCell>
						...
						{(message === '' || message === 'Failed task, try later.') &&
							selectedFile !== null && (
								<Button
									variant='outline'
									size='icon'
